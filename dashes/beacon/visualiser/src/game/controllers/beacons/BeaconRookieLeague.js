...
import BeaconsMapRookieLeagueController from "../maps/MapRookieLeague.js";
 ...
 */
export default class BeaconRookieLeagueController extends CanvasController {
    ...
    this._circleCoordinates.forEach((circle, index) => {
      ...
      if (this._circleSizes[index] < targetSizes[index]) {
        this._circleSizes[index] +=
          targetSizes[index] / BeaconRookieLeagueController.GROWTH_SPEED;
      }
    });
  ...
  #calcStrokeWeight() {
    return Math.max(
      ...
      BeaconRookieLeagueController.MIN_STROKE_WEIGHT,
    );
  }
  ...
  #calculateBeaconDiameter() {
    return Math.min(
      ...
      BeaconRookieLeagueController.MIN_BEACON_DIAMETER,
    );
  }
