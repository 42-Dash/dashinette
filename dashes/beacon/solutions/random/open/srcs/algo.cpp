...
static auto mark_placement(const tuple<int, int, int, int> &placement, vector<vector<SquareStatus>> &lines) -> void {
    _apply_to_coverage_area(get<0>(placement), get<1>(placement), get<3>(placement), lines, [&](int i, int j) {
...
static auto count_placement_score(const int &beacon, const int &row, const int &col, vector<vector<SquareStatus>> &lines) -> tuple<int, int, int, int> {
    ...
    return {row, col, score, beacon + static_cast<int8_t>(lines[row][col])};
}
...
auto find_placement_random(const list<int> &beacons, vector<vector<SquareStatus>> &lines) -> vector<tuple<int, int>> {
    ...
    for (auto &&beacon : beacons) {
        ...
        for (int i = 0; i < TRIES || get<2>(best) == -1; ++i) {
            ...
            if (lines[row][col] == SquareStatus::NODE || static_cast<int8_t>(lines[row][col]) < 0) {
                i--;
                continue;
            }

            auto [r, c, score, radius] = count_placement_score(beacon, row, col, lines);
            if (score > get<2>(best)) {
                best = {r, c, score, radius};
            }

        }
        mark_placement(best, lines);
